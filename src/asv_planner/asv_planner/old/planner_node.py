...
class ASVPathPlanner(Node):
    ...
    def __init__(self):
        ...
        self.zed_detection_topic = str(self.get_parameter("zed_detection_topic").value)
        ...
        # Task 1 with ZED
        # ZED gate detection tuning
        self.zed_min_range = 0.0
        self.zed_max_range = 25.0
        self.zed_min_gate_width = 0.2
        self.zed_max_gate_width = 10.0
        self.zed_body_ahead_min = 1.0
        self.zed_through_dist = 4.5
        self.zed_min_confidence = 0.25
        self.zed_ignore_radius = 6.0
        self.zed_use_nearest_pair = False   # False = use scored best pair
        self.last_zed_gate_log_time = 0.0
        self.zed_gate_log_period = 2.0   # use 2.0 for every 2 seconds
    ...
    def zed_detection_callback(self, msg: ZedDetection):
        """
        Use ZED custom detections to find a red-green buoy gate.

        Input from ZED is assumed LOCAL NWU:
            x = forward
            y = left / west-like
            z = up

        Internally we convert immediately to LOCAL NED:
            x_ned = x_nwu
            y_ned = -y_nwu
            z_ned = -z_nwu

        Then ALL calculations are done in NED.
        """

        if self.current_task not in (TASK_ENTRY_EXIT, TASK_NAV_CHANNEL):
            return

        if not hasattr(self, "last_zed_gate_log_time"):
            self.last_zed_gate_log_time = 0.0
        if not hasattr(self, "zed_gate_log_period"):
            self.zed_gate_log_period = 1.0
        if not hasattr(self, "last_gate_mid"):
            self.last_gate_mid = None
        if not hasattr(self, "zed_max_mid_jump"):
            self.zed_max_mid_jump = 8.0  # meters
        ...
        def body_to_ned(xb, yb, x0, y0, psi):
            """
            BODY -> GLOBAL NED
            Inverse of ned_to_body()
            xb = forward
            yb = left
            """
            c = math.cos(psi)
        ...
        # Current vehicle pose in global NED
        ...
        # ---------------------------------
        # Collect valid buoy detections
        # ---------------------------------
        for i in range(n):
            cls = str(msg.class_name[i]).strip().lower()
            conf = float(msg.confidence[i])

            if conf < self.zed_min_confidence:
                continue

            # ZED INPUT IS NWU
            x_local_nwu = float(msg.x_loc[i])
            y_local_nwu = float(msg.y_loc[i])
            z_local_nwu = float(msg.z_loc[i])

            # Convert LOCAL NWU -> LOCAL NED
            x_local = x_local_nwu
            y_local = -y_local_nwu
            z_local = -z_local_nwu

            rng = math.sqrt(x_local * x_local + y_local * y_local)
            if rng < self.zed_min_range or rng > self.zed_max_range:
                continue

            # Must be in front in LOCAL NED/BODY coordinates
            if x_local <= self.zed_body_ahead_min:
                continue

            # Convert LOCAL NED/BODY -> GLOBAL NED
            xg, yg = body_to_ned(x_local, y_local, x0, y0, psi)
            ...
            entry = {
                "p_global": p,
                "conf": conf,
                "x_local": x_local,   # LOCAL NED
                "y_local": y_local,   # LOCAL NED
                "z_local": z_local,   # LOCAL NED
            }

            if cls == "red_buoy":
                red_buoys.append(entry)
            elif cls == "green_buoy":
                green_buoys.append(entry)
        ...
        for r_data in red_buoys:
            for g_data in green_buoys:
                r = r_data["p_global"]
                g = g_data["p_global"]
                ...
                if width < self.zed_min_gate_width or width > self.zed_max_gate_width:
                    continue

                mid_x = 0.5 * (r.x + g.x)
                mid_y = 0.5 * (r.y + g.y)

                # Midpoint must be ahead in body frame
                mx_b, my_b = ned_to_body(mid_x, mid_y, x0, y0, psi)
                if mx_b <= self.zed_body_ahead_min:
                    continue

                # Prefer closer forward gate, smaller lateral offset, higher confidence
                score = mx_b + 0.25 * abs(my_b) + 0.03 * width
                score -= 0.5 * (r_data["conf"] + g_data["conf"])
        ...
        # ---------------------------------
        # Smooth updates: reject only huge jumps
        # ---------------------------------
        mid_x, mid_y = best_mid
        if self.last_gate_mid is not None:
            jump = math.hypot(mid_x - self.last_gate_mid[0], mid_y - self.last_gate_mid[1])
            if jump > self.zed_max_mid_jump:
                return

        # ---------------------------------
        # Build midpoint + through-gate waypoint
        # ---------------------------------
        r, g = best_pair
        ...
        through_dist = max(4.0, min(self.zed_through_dist, 0.75 * mx_b))

        # Two perpendicular candidates in GLOBAL NED
        perp_x1 = -uy * through_dist
        perp_y1 =  ux * through_dist
        perp_x2 =  uy * through_dist
        perp_y2 = -ux * through_dist

        point1_x = mid_x + perp_x1
        point1_y = mid_y + perp_y1
        point2_x = mid_x + perp_x2
        point2_y = mid_y + perp_y2

        # Pick the candidate that is more forward in BODY frame
        p1_bx, p1_by = ned_to_body(point1_x, point1_y, x0, y0, psi)
        p2_bx, p2_by = ned_to_body(point2_x, point2_y, x0, y0, psi)

        self.x_goal_poses[0] = float(mid_x)
        self.y_goal_poses[0] = float(mid_y)

        if p1_bx >= p2_bx:
            self.x_goal_poses[1] = float(point1_x)
            self.y_goal_poses[1] = float(point1_y)
            chosen_wp2_bx = p1_bx
            chosen_wp2_by = p1_by
        else:
            self.x_goal_poses[1] = float(point2_x)
            self.y_goal_poses[1] = float(point2_y)
            chosen_wp2_bx = p2_bx
            chosen_wp2_by = p2_by

        self.goal_pose_qty = 2
        self.point = 0
        self.found_gates = True

        # Update accepted midpoint only after passing jump check
        self.last_gate_mid = (mid_x, mid_y)

        # ---------------------------------
        # Psi goal
        # ---------------------------------
        mid_local_x_print, mid_local_y_print = ned_to_body(
            self.x_goal_poses[0], self.y_goal_poses[0], x0, y0, psi
        )

        wp2_local_x_print, wp2_local_y_print = ned_to_body(
            self.x_goal_poses[1], self.y_goal_poses[1], x0, y0, psi
        )

        # Heading to the midpoint in LOCAL NED/BODY coordinates
        gate_dir_x = mid_local_x_print
        gate_dir_y = mid_local_y_print
        self.psi_goal_poses[0] = (
            self.pi_wrap(math.atan2(gate_dir_y, gate_dir_x)) + self.psi_usv_NED
        )

        now = time.time()
        if (now - self.last_zed_gate_log_time) >= self.zed_gate_log_period:
            self.last_zed_gate_log_time = now
            self.get_logger().info(
                f"ZED gate selected | "
                f"mid_local_ned=({mid_local_x_print:.2f}, {mid_local_y_print:.2f}) | "
                f"wp2_local_ned=({wp2_local_x_print:.2f}, {wp2_local_y_print:.2f}) | "
                f"mid_global_ned=({self.x_goal_poses[0]:.2f}, {self.y_goal_poses[0]:.2f}) | "
                f"wp2_global_ned=({self.x_goal_poses[1]:.2f}, {self.y_goal_poses[1]:.2f}) | "
                f"mid_body=({mx_b:.2f}, {my_b:.2f}) | "
                f"wp2_body=({chosen_wp2_bx:.2f}, {chosen_wp2_by:.2f}) | "
                f"width={best_width:.2f} | "
                f"through_dist={through_dist:.2f} | "
                f"jump_limit={self.zed_max_mid_jump:.2f} | "
                f"psi_goal={self.psi_goal_poses[0]:.2f}"
            )
    ...
    def waypoints_task1(self):
        ...
        self.x_goal_poses[0] = 0.0
        self.y_goal_poses[0] = 0.0

        self.x_goal_poses[1] = 10.0
        self.y_goal_poses[1] = 0.0

        self.x_goal_poses[2] = 10.0
        self.y_goal_poses[2] = -10.0

        self.x_goal_poses[3] = 0.0
        self.y_goal_poses[3] = -10.0

        self.goal_pose_qty = 4
