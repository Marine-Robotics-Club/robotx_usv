...
class ControlAllocatorDiffDrivePWM(Node):
    ...
    Differential-drive allocation, same convention as your C++ allocator:

        Np_cmd = T_x / 2 + M_z / B
        Ns_cmd = T_x / 2 - M_z / B

    where:
        Np_cmd = desired port/left thrust in Newtons
        Ns_cmd = desired starboard/right thrust in Newtons
    ...
    def __init__(self):
        ...
        # Same meaning as your C++ allocator:
        # Np = Tx/2 + Mz/B
        # Ns = Tx/2 - Mz/B
        #
        # If B is the full lateral distance between thrusters, keep this formula.
        ...
        self.declare_parameter("jetson_cmds_topic", "jetson_cmds")
        ...
        if abs(self.B) < 1e-6:
            self.get_logger().warn("B is too small. Resetting B to 0.3.")
            self.B = 0.3

        # ---------------- Calibration table ----------------
        # Column 0: motor command percentage
        # Column 1: measured thrust in Newtons
        calib = np.array([
            [-100, -155.68], [-90, -128.1024], [-80, -98.7456], [-70, -69.3888],
            [-60, -46.704],  [-50, -30.2464],  [-40, -15.568],  [-30, 0.0],
            [-20, 0.0],      [-10, 0.0],       [0, 0.0],        [10, 0.0],
            [20, 0.0],       [30, 22.24],      [40, 53.376],    [50, 93.408],
            [60, 133.44],    [70, 191.264],    [80, 240.192],   [90, 289.12],
            [100, 346.944]
        ], dtype=float)
        ...
        fwd_mask = self._pct >= 30
        rev_mask = self._pct <= -40

        self._thrN_fwd = self._thrN[fwd_mask]     # 22.24 to 346.944
        self._pct_fwd = self._pct[fwd_mask]       # 30 to 100

        # Reverse side must be sorted by thrust value for np.interp
        order = np.argsort(self._thrN[rev_mask])
        self._thrN_rev = self._thrN[rev_mask][order]   # -155.68 to -15.568
        ...
        self.get_logger().info(
            ...
            f"  B: {self.B:.3f}\n"
            f"  Forward thrust range: {self._N_fwd_min:.2f} to {self._N_fwd_max:.2f} N\n"
            f"  Reverse thrust range: {self._N_rev_min:.2f} to {self._N_rev_max:.2f} N\n"
    ...
    def _tau_cb(self, msg: ControlEffort):
        ...
        # Convert thrust command in Newtons to PWM through calibration.
    ...
    def _thrustN_to_pwm(self, N: float) -> int:
        """
        Convert desired thrust in Newtons to PWM using calibration.

        Deadband from calibration:
            reverse deadband: approximately -15.568 N to 0 N -> 1500 us
            forward deadband: 0 N to 22.24 N -> 1500 us

        The function also saturates to the available calibrated thrust range.
        """
        ...
        if N > 0.0:
            ...
            # Example: -10 N should still be neutral.
