...
def apf_tau_wall_following(
    *,
    ...
    semantic_bias_max_deg: float = 25.0,
    ka: float = 1.8,
    kr: float = 40.0,
    rho: float = 25.0,
    obs_rad=None,
    ...
    theta_bias_max = math.radians(max(0.0, float(semantic_bias_max_deg)))
    ...
    for o_b, r_o, color in zip(obs_b, radii, colors):
        ...
        if d <= 0.0:
            # Safety overrides semantics inside the inflated boundary: leave
            # the emergency repulsion purely radial.
            f_rep += (kr * 1e3) * n_hat

        elif d < rho:
            n_rep = n_hat

            # Semantic passing-side bias. We rotate ONLY the direction of
            # the existing nonlinear repulsive force; its magnitude, safety
            # boundary, rho, kr, and all downstream APF logic are unchanged.
            #
            # Body frame: +x forward, +y right. For an obstacle ahead:
            #   green -> positive rotation of outward repulsion -> vehicle left
            #            -> green buoy remains on the vehicle's right/starboard
            #   red   -> negative rotation -> vehicle right
            #            -> red buoy remains on the vehicle's left/port
            if semantic_bias_enabled and color in ("green", "red") and rho > 1e-9:
                proximity = float(np.clip((rho - d) / rho, 0.0, 1.0))

                # Same distance-dependent idea as the earlier biased APF:
                # bias grows as the buoy gets closer, with a configurable cap.
                theta_mag = min(theta_bias_max, math.pi * proximity)
                theta_bias = theta_mag if color == "green" else -theta_mag

                cb = math.cos(theta_bias)
                sb = math.sin(theta_bias)
                R_bias = np.array(
                    [[cb, -sb], [sb, cb]],
                    dtype=np.float64,
                )
...
def apf_tau_diff_drive_alignment(
    *,
    ...
    _tau_full, dbg = apf_tau_wall_following(
        ...
        semantic_bias_max_deg=semantic_bias_max_deg,
        ka=ka,
        kr=kr,
        rho=rho,
        obs_rad=obs_rad,
...
class APF_SKC_Controller(Node):
    def __init__(self):
        ...
        self.declare_parameter("semantic_bias_max_deg", 25.0)
        ...
        self.semantic_bias_max_deg = float(self.get_parameter("semantic_bias_max_deg").value)
        ...
        self.get_logger().info(
            f"Semantic buoy bias: enabled={self.use_semantic_bias}, "
            f"topic={semantic_full_topic}, match_radius={self.semantic_match_radius_m:.2f} m, "
            f"max_bias={self.semantic_bias_max_deg:.1f} deg"
        )
    ...
    def _associate_obstacle_colors(self, obstacles_w: np.ndarray) -> List[str]:
        ...
        the same repulsive magnitude, but its direction can be biased.
        """
    ...
    def _compute_lookahead_yaw_error(
        ...
        for _ in range(n):
            ...
            _tau_i, dbg = apf_tau_wall_following(
                ...
                semantic_bias_max_deg=self.semantic_bias_max_deg,
                ka=self.ka,
                kr=self.kr,
                rho=self.rho,
                r_usv=self.r_usv,
