...
from geometry_msgs.msg import Point, Twist, TwistStamped, Pose2D
...
class WamVSysID(Node):
    def __init__(self):
        ...
        # --- publishers ---
        self.Motor_cmds_pub = self.create_publisher(MotorCmds, f'/{self.wamv_name}/motor_cmds', 1)
        ...
        self._log_csv.writerow([
            'sim_time_s','ros_time_s','wall_time_s','test_type',
            'x','y','yaw',
            'u','v','vz',
            'wx','wy','r','ax','ay','r_dot',
        ...
        self.sim_t0 = None    # first sim time when we start logging
    ...
    def _log_sample(self):
        ...
        now = self.get_clock().now()   # this is sim time if use_sim_time = True
        ros_t = now.nanoseconds * 1e-9

        # initialize reference sim time
        ...
        row = [
            f(sim_t, 3), f(ros_t, 3), f(wall_t, 3), self.test_type,
    ...
    def loop(self):
        ...
        if self.test_type == 'bollard_pull':
            ...
            if (time.time() - self.timer_test) > 15.0:
