...
private:
    ...
    void declare_parameters() {
        ...
        this->declare_parameter<double>("min_detect_height", 0.0);
        this->declare_parameter<double>("max_detect_height", 3.0);
        ...
        for (const auto& indices : cluster_indices) {
            ...
            if (width_x >= min_x_size_ && max_x_size_ >= width_x 
                ...
                // If the cluster meets the size criteria, add it to the output cloud
                    ...
                    // Filter by centroid height (z)
				if (centroid[2] < min_detect_height_ || centroid[2] > max_detect_height_) {
					continue;  // skip clusters too low or too high
				}
    ...
    double min_detect_dist_, max_detect_dist_, cluster_tolerance_, max_detect_height_, min_detect_height_;
