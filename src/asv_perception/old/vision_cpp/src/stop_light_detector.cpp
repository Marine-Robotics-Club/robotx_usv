...
public:
    PointCloudClusterer() : Node("stop_light_detector") {
        declare_parameters();
        ...
        buoy_detected_pub_= this->create_publisher<lidar_msgs::msg::BuoyDetected>("/vision/output/stop_light_detected",0);
    }
...
private:
    ...
    void declare_parameters() {
        ...
        this->declare_parameter<double>("min_x_size", 0.1);
        this->declare_parameter<double>("min_y_size", 0.1);
        this->declare_parameter<double>("min_z_size", 0.1);
        this->declare_parameter<double>("max_x_size", 0.6);
        this->declare_parameter<double>("max_y_size", 0.6);
        this->declare_parameter<double>("max_z_size", 0.6);
    ...
    void pointcloud_callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
        ...
        for (const auto& indices : cluster_indices) {
            ...
            if (width_x >= min_x_size_ && max_x_size_ >= width_x 
                ...
                for (auto& point : cluster->points) { //change for color wanting 
                    point.r = 255;
                    point.g = 0;
                    point.b = 0;
                }
		...
		buoy_detected_msg.name.push_back("Stop Light[" + std::to_string(cluster_id) + "]");
