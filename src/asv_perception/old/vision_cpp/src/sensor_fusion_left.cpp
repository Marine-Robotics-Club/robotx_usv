...
//NEED TO MAKE SURE OUTPUT IS IN NWU so for left it is -90 deg rot z 


class SensorFusionNode : public rclcpp::Node
{
public:
    SensorFusionNode() : Node("sensor_fusion_node_left")
    {
        ...
        yolo_sub_ = this->create_subscription<yolov26_msgs::msg::YoloDetection>(
            "/yolov9/left/detections", 10, std::bind(&SensorFusionNode::yolo_callback, this, std::placeholders::_1));
...
private:
        ...
        for (size_t i = 0; i < objects.class_name.size(); ++i) {
            ...
            for (size_t j = 0; j < buoy.object_names.size(); ++j) {
                if (objects.class_name[i] == "green_pole_buoy" || objects.class_name[i] == "red_pole_buoy") {
                    ...
                    double left_pixel = ((1280.0 / -max_angle_2) * left_angle) + 1920;
                    double right_pixel = ((1280.0 / -max_angle_2) * right_angle) + 1920;

                    if ((objects.x_center[i] >= left_pixel) && (objects.x_center[i] <= right_pixel)) {
                        RCLCPP_INFO(this->get_logger(), "LSF: Pole Buoy match found!!! xmin: %f xmax: %f", left_pixel, right_pixel);
            ...
            for (size_t j = 0; j < ball.object_names.size(); ++j) {
                if (objects.class_name[i] == "black_buoy" || objects.class_name[i] == "blue_bouy" || objects.class_name[i] == "green_bouy" || objects.class_name[i] == "yellow_bouy" || objects.class_name[i] == "red_bouy") {
                    ...
                    if ((objects.x_center[i] >= left_pixel) && (objects.x_center[i] <= right_pixel)) {
                        RCLCPP_INFO(this->get_logger(), "LSF: BALL match found!!! xmin: %f xmax: %f", left_pixel, right_pixel);
                        fusion.object_names.push_back(objects.class_name[i]);
            ...
            for (size_t j = 0; j < boat.object_names.size(); ++j) {
                if (objects.class_name[i] == "orange_boat" || objects.class_name[i] == "black_boat") {
                    double search_size = 1.5;
                    ...
                    if ((objects.x_center[i] >= left_pixel) && (objects.x_center[i] <= right_pixel)) {
                        RCLCPP_INFO(this->get_logger(), "LSF: BOAT match found!!! xmin: %f xmax: %f", left_pixel, right_pixel);
                        fusion.object_names.push_back(objects.class_name[i]);
            ...
            for (size_t j = 0; j < stop_light.object_names.size(); ++j) {
                if (objects.class_name[i] == "green_light" || objects.class_name[i] == "red_light") {
                    ...
                    if ((objects.x_center[i] >= left_pixel) && (objects.x_center[i] <= right_pixel)) {
                        RCLCPP_INFO(this->get_logger(), "LSF: stop_light match found!!! xmin: %f xmax: %f", left_pixel, right_pixel);
                        fusion.object_names.push_back(objects.class_name[i]);
    ...
    fau_msgs::msg::ObjectPositionArray dock;
