...
public:
    PointCloudClusterer() : Node("boat_detector") {
        declare_parameters();
        ...
        buoy_detected_pub_= this->create_publisher<lidar_msgs::msg::BuoyDetected>("/vision/output/boat_detected",1);
    }
...
private:
    ...
    void pointcloud_callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
        ...
        for (const auto& indices : cluster_indices) {
            ...
            if (width_x >= min_x_size_ && max_x_size_ >= width_x 
                ...
                for (auto& point : cluster->points) {
                    point.r = 255; // change for color
                    point.g = 19;
                    point.b = 240; //PINK NEON :)
                }
		...
		buoy_detected_msg.name.push_back("Boat[" + std::to_string(cluster_id) + "]");
