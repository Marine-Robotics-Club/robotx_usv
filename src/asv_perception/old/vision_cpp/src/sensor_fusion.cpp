...
public:
    SensorFusionNode() : Node("sensor_fusion_node")
    {
        ...
        yolo_sub_ = this->create_subscription<yolov26_msgs::msg::YoloDetection>(
            "/yolov9/center/detections", 10, std::bind(&SensorFusionNode::yolo_callback, this, std::placeholders::_1));
        ...
        max_length_ = 50.0;       // Maximum distance from the LiDAR
    }
        ...
        for (size_t i = 0; i < objects.class_name.size(); ++i) {
            ...
            for (size_t j = 0; j < buoy.object_names.size(); ++j) {
                if (objects.class_name[i] == "green_pole_buoy" || objects.class_name[i] == "red_pole_buoy") {
                    ...
                    double left_angle = atan2((buoy.y_object[j] + search_size), buoy.x_object[j]);
                    double right_angle = atan2((buoy.y_object[j] - search_size), buoy.x_object[j]);
                    ...
                    if ((objects.x_center[i] >= left_pixel) && (objects.x_center[i] <= right_pixel)) {
                        RCLCPP_INFO(this->get_logger(), "SF: Pole Buoy match found!!! xmin: %f xmax: %f", left_pixel, right_pixel);
            ...
            for (size_t j = 0; j < ball.object_names.size(); ++j) {
                if (objects.class_name[i] == "black_buoy" || objects.class_name[i] == "blue_bouy" || objects.class_name[i] == "green_bouy" || objects.class_name[i] == "yellow_bouy" || objects.class_name[i] == "red_bouy") {
                    double search_size = 1.5;
                    double left_angle = atan2((ball.y_object[j] + search_size), ball.x_object[j]);
                    double right_angle = atan2((ball.y_object[j] - search_size), ball.x_object[j]);
                    double left_pixel = ((1280.0 / -max_angle_2) * left_angle) + 640;
                    double right_pixel = ((1280.0 / -max_angle_2) * right_angle) + 640;

                    if ((objects.x_center[i] >= left_pixel) && (objects.x_center[i] <= right_pixel)) {
                        RCLCPP_INFO(this->get_logger(), "SF: BALL match found!!! xmin: %f xmax: %f", left_pixel, right_pixel);
                        fusion.object_names.push_back(objects.class_name[i]);
            ...
            for (size_t j = 0; j < boat.object_names.size(); ++j) {
                if (objects.class_name[i] == "black_cross" || objects.class_name[i] == "black_triangle") {
                    double search_size = 1.5;
                    double left_angle = atan2((boat.y_object[j] + search_size), boat.x_object[j]);
                    double right_angle = atan2((boat.y_object[j] - search_size), boat.x_object[j]);
                    double left_pixel = ((1280.0 / -max_angle_2) * left_angle) + 640;
                    double right_pixel = ((1280.0 / -max_angle_2) * right_angle) + 640;

                    if ((objects.x_center[i] >= left_pixel) && (objects.x_center[i] <= right_pixel)) {
                        RCLCPP_INFO(this->get_logger(), "SF: BOAT match found!!! xmin: %f xmax: %f", left_pixel, right_pixel);
                        fusion.object_names.push_back(objects.class_name[i]);
            ...
            for (size_t j = 0; j < stop_light.object_names.size(); ++j) {
                if (objects.class_name[i] == "green_light" || objects.class_name[i] == "red_light") {
                    double search_size = 1.5;
                    double left_angle = atan2((stop_light.y_object[j] + search_size), stop_light.x_object[j]);
                    double right_angle = atan2((stop_light.y_object[j] - search_size), stop_light.x_object[j]);
                    double left_pixel = ((1280.0 / -max_angle_2) * left_angle) + 640;
                    double right_pixel = ((1280.0 / -max_angle_2) * right_angle) + 640;

                    if ((objects.x_center[i] >= left_pixel) && (objects.x_center[i] <= right_pixel)) {
                        RCLCPP_INFO(this->get_logger(), "SF: stop_light match found!!! xmin: %f xmax: %f", left_pixel, right_pixel);
                        fusion.object_names.push_back(objects.class_name[i]);
